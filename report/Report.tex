...
Instantaneous heart rate was computed as the inverse of successive R-R intervals $\frac{60}{RR}$ (BPM), where RR is the time interval between two consecutive R-peaks. Then it's resampled onto a common 4Hz grid via interpolation to allow comparison across subjects.\\
...
In order to compute ISC-HR, all subjects' interpolated HR series were truncated to their shortest common length and arranged into a matrix of pairwise Pearson correlations.\\
Each subject's ISC-HR was computed as the FIsher-Z average of their correlation with every other subject in the group, excluding self-correlation. This is followed by the inverse Fisher-Z transform.
